...
class Results(ABC):
    ...
    def create_array(self) -> np.ndarray:
        ...
        affected_cells_floor = [
            (i[0] - r_min, i[1] - c_min) for i in self.affected_cells
        ]
...
class FlowResults(Results):
    def __init__(
        self, controls: Controls, affected_cells: list, cell_values: list
    ) -> None:
        self.controls = controls
        self.affected_cells = affected_cells
