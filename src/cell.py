...
class Cell:
    def __init__(
        self, controls: Controls, coords: Tuple[int, int], previous_direction: int = 0
    ) -> None:
    ...
    def __neighbour_coords(self) -> list:
        ...
        coords = [
            (i, j) for i in list(range(r_min, r_max)) for j in list(range(c_min, c_max))
        ]
    ...
    def __on_boundary(self) -> bool:
        b = (
            False
            if self.__controls.dem.boundary[self.coords[0], self.coords[1]] == 0
            else True
        )
        return b
    ...
    def __neighbour_tan_slopes(self) -> list:
        neighbour_values_calc = [
            i - 0.01 if i == self.value and self.__controls.horizontal_flow else i
            for i in self.neighbour_values
        ]
        tan_slopes = [
            ((self.value - neighbour_value) / abs(self.__controls.dem.resolution[0]))
    ...
    def __neighbour_yis(self) -> list:
        neighbour_yis = [
            i / np.tan(self.__controls.slope_threshold * _RAD_TO_DEG)
            if i is not None
            else 0
